...
VERSION="${2:-0.3.26-alpha}"
RAW="/tmp/PU2PNY-${VERSION}-validate-0326.img"
ROOT="/mnt/pu2pny-os-0326"
LOOP=""; PID=""
cleanup(){
  set +e
...
grep -Fq 'nmcli --wait 14 connection up PU2PNY-WIFI-CANDIDATE' "$ROOT/usr/local/sbin/2pny-network-switch"
grep -Fq 'ipv4.dhcp-timeout 12' "$ROOT/usr/local/sbin/2pny-network-switch"
...
grep -Fq 'nm --wait 6 connection up "$chosen"' "$ROOT/usr/local/sbin/2pny-wifi-profiles"
...
grep -Fq "PNY.operation('Reiniciando hotspot'" "$SYS"
...
echo "PU2PNY-OS $VERSION ARM64 image: focused MMDVM/Wi-Fi SW/structural gates PASS; real MMDVM recognition and Wi-Fi 1->2 failover remain HW PENDING"
