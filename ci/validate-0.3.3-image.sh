...
VERSION="${2:-0.3.3-alpha}"
RAW="/tmp/PU2PNY-${VERSION}-validate.img"
ROOT="/mnt/pu2pny-os-033"
LOOP=""; PID=""
cleanup(){
  set +e
...
chroot "$ROOT" /usr/local/bin/2pnyd >/tmp/pu2pnyd-033.log 2>&1 & PID=$!
OK=0
for _ in {1..100}; do
...
echo "PU2PNY-OS $VERSION ARM64 image: OK — complete menu, reboot Wi-Fi handoff, mDNS, D-Star/YSF assets and live UI validated"