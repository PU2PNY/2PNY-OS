...
VERSION="${2:-0.3.1-alpha}"
RAW="/tmp/PU2PNY-${VERSION}-validate.img"
ROOT="/mnt/pu2pny-os-031"
LOOP=""; PID=""
cleanup(){
  set +e
...
HOSTS="$ROOT/var/lib/2pny/hostfiles"
for x in DStar_Hosts.json DPlus_Hosts.txt DExtra_Hosts.txt DCS_Hosts.txt XLXHosts.txt YSFHosts.txt P25Hosts.txt NXDNHosts.txt; do
  test -s "$HOSTS/$x"
done
...
chroot "$ROOT" /usr/local/bin/2pnyd >/tmp/pu2pnyd-031.log 2>&1 & PID=$!
OK=0
for _ in {1..100}; do
  if curl -fsS http://127.0.0.1/healthz 2>/dev/null | grep -q 'PU2PNY OK'; then OK=1; break; fi
  sleep .2
done
test "$OK" = 1 || { cat /tmp/pu2pnyd-031.log; exit 1; }
curl -fsS http://127.0.0.1/api/status | grep -Fq '"version":"0.3.1-alpha"'
