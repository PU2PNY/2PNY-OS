...
strings "$ROOT/usr/local/bin/2pnyd" | grep -Fq '0.3.15-alpha'
...
echo '[14h/15] inherited TGIF/i18n + 0.3.15 focused onboarding gates'
RF="$ROOT/usr/local/sbin/2pny-rf-apply"
...
echo "PU2PNY-OS $VERSION ARM64 image: 0.3.15 focused MQTT-onboarding correction + inherited structural gates OK — hardware validation still required"
chroot "$ROOT" /usr/sbin/mosquitto -c /etc/mosquitto/2pny-local.conf >/tmp/pu2pny-mosquitto-test.log 2>&1 & PID=$!
MQTT_OK=0
for _ in {1..40}; do
  if python3 - <<'PYMQTT'
