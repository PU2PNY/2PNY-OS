...
# Deterministic setup networking: exactly one setup transport owns 10.42.0.1.
...
# Fast first boot. Before provisioning, setup address is always 10.42.0.1.
...
if [[ -f "$STATE/provisioned" ]]; then
  nmcli connection modify 2PNY-Ethernet connection.autoconnect yes 2>/dev/null || true
  stamp '2PNY: provisioned'
  exit 0
fi
...
ETH_CARRIER=0
if [[ -n "${ETH_IF:-}" && -r "/sys/class/net/$ETH_IF/carrier" ]]; then read -r ETH_CARRIER <"/sys/class/net/$ETH_IF/carrier" || ETH_CARRIER=0; fi
nmcli connection down 2PNY-Ethernet >/dev/null 2>&1 || true
nmcli connection down 2PNY-SETUP-ETH >/dev/null 2>&1 || true
nmcli connection down 2PNY-SETUP >/dev/null 2>&1 || true
if [[ "$ETH_CARRIER" == 1 ]]; then
  if nmcli --wait 8 connection up 2PNY-SETUP-ETH ifname "$ETH_IF" >/dev/null 2>&1; then
    stamp '2PNY: setup ready on Ethernet http://10.42.0.1'
    exit 0
  fi
fi
if [[ -n "${WIFI_IF:-}" ]]; then
  ip link set "$WIFI_IF" up 2>/dev/null || true
  if nmcli --wait 8 connection up 2PNY-SETUP ifname "$WIFI_IF" >/dev/null 2>&1; then
    stamp '2PNY: setup ready on Wi-Fi http://10.42.0.1'
    exit 0
  fi
  ...
  if nmcli --wait 8 device wifi hotspot ifname "$WIFI_IF" con-name 2PNY-SETUP ssid 2PNY-SETUP band bg channel 6 password 2pnysetup >/dev/null 2>&1; then
    nmcli connection modify 2PNY-SETUP ipv4.method shared ipv4.addresses 10.42.0.1/24 connection.autoconnect no connection.mdns yes >/dev/null 2>&1 || true
    stamp '2PNY: setup recovered on Wi-Fi http://10.42.0.1'
    exit 0
  fi
fi
stamp '2PNY ERROR: setup transport unavailable'
exit 1
''')

# Dispatcher: while unprovisioned, restore setup if the active setup transport disappears.
...
case "$ACTION" in
  down|connectivity-change)
    systemctl restart 2pny-firstboot.service >/dev/null 2>&1 || true
    ;;
esac
exit 0
''')

# Make 2pnyd start independently of network-online and keep local HTTP available.
svc=root/'rootfs-overlay/etc/systemd/system/2pnyd.service'
if svc.exists():
    s=svc.read_text()
    s=s.replace('After=NetworkManager.service network-online.target','After=NetworkManager.service')
    s=s.replace('Wants=NetworkManager.service network-online.target','Wants=NetworkManager.service')
    svc.write_text(s)

# Patch setup backend/UI: no absolute redirect to mDNS; acknowledge before changing transport.
p=root/'src/2pnyd/main.go'
s=p.read_text()
s=s.replace('http://2pny.local/wizard','/wizard')
s=s.replace('http://2pny.local/','/wizard')
s=s.replace('http://2pny.local','/wizard')
# Browser must never leave a working setup host because of mDNS.
s=s.replace("window.location.href='/wizard/wizard'", "window.location.href='/wizard'")
s=s.replace("window.location.href='/wizard'", "window.location.href='/wizard'")
# Replace the Wi-Fi transition helper with deterministic setup teardown/recovery.
pat=re.compile(r'func connectConfiguredWiFi\(ssid,password string\) error \{.*?\n\}', re.S)
new=r'''func connectConfiguredWiFi(ssid,password string) error {
	...
	args:=[]string{"--wait","20","device","wifi","connect",ssid,"ifname",iface,"name","2PNY-WIFI"}; if password!=""{args=append(args,"password",password)}
	b,err:=exec.Command("nmcli",args...).CombinedOutput(); if err!=nil{
		msg:=strings.TrimSpace(string(b)); if msg==""{msg=err.Error()}
		exec.Command("systemctl","restart","2pny-firstboot.service").Run()
...
if pat.search(s): s=pat.sub(lambda _m:new,s,count=1)

# Ethernet-only provisioning changes from setup shared IP to normal DHCP only after response.
old='''writeSetupState("connected","Configuração concluída pela Ethernet.")'''
if old in s:
    s=s.replace(old,'''writeSetupState("connecting","Configuração salva. Mudando a Ethernet para a rede normal...")''',1)
# Make success copy explicit and do not promise mDNS redirect.
s=s.replace('Abrindo o painel automaticamente...', 'Configuração salva. Reconecte à sua rede normal e abra o IP atribuído ao 2PNY.')
...
# Ensure wizard link itself is relative.
s=s.replace('href="http://2pny.local/wizard"','href="/wizard"')
p.write_text(s)

# Source validation for the exact first-access contract.
...
test -s rootfs-overlay/etc/NetworkManager/system-connections/2pny-setup-ethernet.nmconnection
'''
if '# 2PNY_FIRST_ACCESS_0_1_6' not in vs: vs += block
v.write_text(vs)
PY
