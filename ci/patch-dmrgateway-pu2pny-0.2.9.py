#!/usr/bin/env python3
"""Patch pinned upstream DMRGateway with PU2PNY RF group controls.
...
block='''			} else if (flco == FLCO::GROUP && slotNo == m_xlxSlot && m_xlxUserControl && dstId >= 4000U && dstId <= 4026U) {
				// PU2PNY easy RF control: TG4000 unlinks and TG4001..TG4026
				// select XLX modules A..Z.  Only the terminator changes state,
				...
				timer[slotNo]->start();
			} else if (flco == FLCO::GROUP && slotNo == m_xlxSlot && m_xlxUserControl && dstId == 4099U) {
