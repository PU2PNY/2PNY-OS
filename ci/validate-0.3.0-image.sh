...
VERSION="${2:-0.3.0-alpha}"
RAW="/tmp/PU2PNY-${VERSION}-validate.img"
ROOT="/mnt/pu2pny-os-030"
LOOP=""; PID=""
cleanup(){
  set +e
...
for x in 2pny-station-worker 2pny-display-core 2pny-display-apply 2pny-server-catalog 2pny-protocol-network-apply 2pny-aprs 2pny-netdiag 2pny-nextion-autodetect; do
...
grep -Fq "location.replace(base+'/wizard?step=2')" "$WIZ"
! grep -Fq 'reconnectManual' "$WIZ"
grep -Fq 'radio_reset' "$SW"
grep -Fq 'systemctl restart avahi-daemon.service' "$SW"
...
grep -Fq 'Aguardando transmissão' "$DASH"
grep -Fq 'renderActivity' "$DASH"
grep -Fq 'class="radar"' "$DASH"
! grep -Fq 'Operador não identificado' "$DASH"
! grep -Fq 'Um único box acompanha' "$DASH"
...
for svc in 2pny-dmrgateway.service 2pny-dstargateway.service 2pny-ysfgateway.service 2pny-p25gateway.service 2pny-nxdngateway.service 2pny-dapnetgateway.service 2pny-display-core.service 2pny-station.service 2pny-aprs.service 2pny-netdiag.service 2pny-mdns-guard.service; do
...
grep -Fq 'Mais usado' "$ROOT/usr/share/2pny/history.html"
...
chroot "$ROOT" /usr/local/bin/2pnyd >/tmp/pu2pnyd-030.log 2>&1 & PID=$!
OK=0
for _ in {1..80}; do
