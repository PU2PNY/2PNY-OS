...
strings "$ROOT/usr/local/bin/2pnyd" | grep -Fq '0.3.14-alpha'
...
echo '[14i/15] 0.3.14 MQTT recovery + wizard retention gates'
RF="$ROOT/usr/local/sbin/2pny-rf-apply"
MQTT="$ROOT/usr/local/sbin/2pny-mqtt-preflight"
...
WIZ="$ROOT/usr/share/2pny/wizard.html"
LANG="$ROOT/usr/share/2pny/ui-language.js"
OP="$ROOT/usr/local/sbin/2pny-operational-apply"
...
grep -Fq 'User=mosquitto' "$MOSQSVC"
grep -Fq 'Restart=on-failure' "$MOSQSVC"
grep -Fq -- '--ensure-service' "$MQTT"
grep -Fq 'ensure_service' "$MQTT"
grep -Fq 'service_snapshot' "$MQTT"
grep -Fq 'systemctl","reset-failed"' "$MQTT"
grep -Fq 'systemctl","start"' "$MQTT"
grep -Fq -- '--ensure-service --attempts 12 --delay 0.4 --service-wait 10' "$RF"
grep -Fq '"--ensure-service","--attempts","12","--delay","0.4","--service-wait","10"' "$OP"
grep -Fq '"--ensure-service","--attempts","12","--delay","0.4","--service-wait","10"' "$PROTO"
grep -Fq "saveBasicDraft(data)" "$WIZ"
grep -Fq "resumeBasic=!alreadyProvisioned" "$WIZ"
grep -Fq "step(3);localStorage.setItem('pu2pny-wizard-step','3')" "$WIZ"
grep -Fq "clearBasicDraft();await loadConclusion();step(4)" "$WIZ"
grep -Fq 'A última tentativa não foi concluída.' "$LANG"
strings "$ROOT/usr/local/bin/2pnyd" | grep -Fq '0.3.14-alpha'

echo "PU2PNY-OS $VERSION ARM64 image: inherited regressions + 0.3.14 MQTT/wizard corrective SW/structural gates OK — hardware validation still required"
