...
strings "$ROOT/usr/local/bin/2pnyd" | grep -Fq '0.3.12-alpha'
...
grep -Fq 'MMDVMHost não iniciou; a configuração anterior foi restaurada' "$RF"
...
test -s "$ROOT/etc/mosquitto/conf.d/2pny-local.conf"
grep -Fq 'listener 1883 127.0.0.1' "$ROOT/etc/mosquitto/conf.d/2pny-local.conf"
grep -Fq 'allow_anonymous true' "$ROOT/etc/mosquitto/conf.d/2pny-local.conf"
...
echo "PU2PNY-OS $VERSION ARM64 image: inherited regressions + 0.3.12 corrective SW/structural gates OK — hardware validation still required"