...
assert data==b"DMRD-A-TO-B",data
b.sendto(b"DMRD-B-TO-A",("127.0.0.1",B_GW))
data,_=a.recvfrom(4096)
assert data==b"DMRD-B-TO-A",data
a.close();b.close()
PY
}

# Direct path + real loopback radio-frame bridge.
start_clients
pair_both
...
for p in (44371,44372):
    with urllib.request.urlopen(f"http://127.0.0.1:{p}/status",timeout=2) as r:s=json.load(r)
    assert s["status"]=="idle" and s["path"]=="Offline" and s["radio_active"] is False,s
PY

# Restart with persisted pair identities and force A through encrypted relay.
...
# Private keys never appear in the public API and local identity stays 0600.
