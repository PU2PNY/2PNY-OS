...
chroot "$ROOT" /usr/local/bin/2pnyd >/tmp/pu2pnyd-037.log 2>&1 & PID=$!
OK=0
for _ in {1..100}; do
  if curl -fsS http://127.0.0.1/healthz 2>/dev/null | grep -q 'PU2PNY OK'; then OK=1; break; fi
  sleep .2
done
test "$OK" = 1 || { cat /tmp/pu2pnyd-037.log; exit 1; }
...
echo '[14b/15] 0.3.7 Direct / D-Star audio / Moderno V2 gates'
...
echo "PU2PNY-OS $VERSION ARM64 image: 0.3.6 regressions + 0.3.7 Direct/D-Star/Display SW/structural OK — hardware validation still required"