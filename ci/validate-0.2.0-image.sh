...
sudo chroot "$ROOT" /usr/local/bin/2pnyd >/tmp/pu2pnyd-020.log 2>&1 & PID=$!
OK=0
for _ in {1..50}; do
...
test "$OK" = 1 || { cat /tmp/pu2pnyd-020.log; exit 1; }
sudo ss -ltn | grep -E '(0\.0\.0\.0|\*|\[::\]):80\b'

echo '[11/12] Runtime APIs and wizard'
curl -fsSI http://127.0.0.1/ | grep -Eq 'HTTP/[0-9.]+ 302'
curl -fsS http://127.0.0.1/wizard >/tmp/pu2pny-wizard-020.html
grep -Fq 'Primeiro acesso — Internet' /tmp/pu2pny-wizard-020.html
grep -Fq "$VERSION" /tmp/pu2pny-wizard-020.html
