...
VERSION="${2:-0.3.4-alpha}"
RAW="/tmp/PU2PNY-${VERSION}-validate.img"
ROOT="/mnt/pu2pny-os-034"
LOOP=""; PID=""
cleanup(){
  set +e
...
for x in 2pny-station-worker 2pny-hardware-probe 2pny-display-core 2pny-display-apply 2pny-server-catalog 2pny-protocol-network-apply 2pny-mqtt-preflight 2pny-aprs 2pny-netdiag 2pny-nextion-autodetect; do
...
chroot "$ROOT" /usr/local/bin/2pnyd >/tmp/pu2pnyd-034.log 2>&1 & PID=$!
OK=0
for _ in {1..100}; do
  if curl -fsS http://127.0.0.1/healthz 2>/dev/null | grep -q 'PU2PNY OK'; then OK=1; break; fi
  sleep .2
done
test "$OK" = 1 || { cat /tmp/pu2pnyd-034.log; exit 1; }
curl -fsS http://127.0.0.1/api/status | grep -Fq '"version":"0.3.4-alpha"'
