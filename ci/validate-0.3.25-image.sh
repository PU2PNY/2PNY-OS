...
VERSION="${2:-0.3.25-alpha}"
RAW="/tmp/PU2PNY-${VERSION}-validate-0325.img"
ROOT="/mnt/pu2pny-os-0325"
LOOP=""; PID=""
cleanup(){
  set +e
...
test "$(sha256sum "$ROOT/usr/local/sbin/2pny-network-switch" | awk '{print $1}')" = "748ab6a9dd7f1f6f31da6a960810f210d1945524938ce9413d9a7f9a93832aff"
...
chroot "$ROOT" /usr/local/bin/2pnyd >/tmp/pu2pnyd-0325.log 2>&1 & PID=$!
OK=0
for _ in {1..100}; do
  if curl -fsS http://127.0.0.1/healthz 2>/dev/null | grep -q 'PU2PNY OK'; then OK=1; break; fi
  sleep .2
done
test "$OK" = 1 || { cat /tmp/pu2pnyd-0325.log; exit 1; }
...
echo "PU2PNY-OS $VERSION ARM64 image: network-only SW/structural gates PASS; protected RF/protocol/clock behavior requires HW regression confirmation"
