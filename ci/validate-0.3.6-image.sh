...
VERSION="${2:-0.3.6-alpha}"
RAW="/tmp/PU2PNY-${VERSION}-validate.img"
ROOT="/mnt/pu2pny-os-036"
LOOP=""; PID=""
cleanup(){
  set +e
...
chroot "$ROOT" /usr/local/bin/2pnyd >/tmp/pu2pnyd-036.log 2>&1 & PID=$!
OK=0
for _ in {1..100}; do
  if curl -fsS http://127.0.0.1/healthz 2>/dev/null | grep -q 'PU2PNY OK'; then OK=1; break; fi
  sleep .2
done
test "$OK" = 1 || { cat /tmp/pu2pnyd-036.log; exit 1; }
curl -fsS http://127.0.0.1/api/status | grep -Fq '"version":"0.3.6-alpha"'
...
grep -Fq '[Gateway]' "$ROOT/usr/local/sbin/2pny-protocol-network-apply"
grep -Fq '[Repeater_1]' "$ROOT/usr/local/sbin/2pny-protocol-network-apply"
grep -Fq '[HostsFiles]' "$ROOT/usr/local/sbin/2pny-protocol-network-apply"
