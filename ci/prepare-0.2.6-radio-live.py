...
install("src/2pnyd-main-0.2.6.go","src/2pnyd/main.go",0o644)
install("src/2pny-network-switch-0.2.6","rootfs-overlay/usr/local/sbin/2pny-network-switch",0o755)
install("src/2pny-rf-apply-0.2.6","rootfs-overlay/usr/local/sbin/2pny-rf-apply",0o755)
install("src/2pny-protocol-network-apply-0.2.6.py","rootfs-overlay/usr/local/sbin/2pny-protocol-network-apply",0o755)
install("src/2pny-server-catalog-0.2.6.py","rootfs-overlay/usr/local/sbin/2pny-server-catalog",0o755)
install("src/2pny-live-status-0.2.6.py","rootfs-overlay/usr/local/sbin/2pny-live-status",0o755)
install("src/2pny-display-apply-0.2.6.py","rootfs-overlay/usr/local/sbin/2pny-display-apply",0o755)
install("src/2pny-display-status-0.2.6.py","rootfs-overlay/usr/local/sbin/2pny-display-status",0o755)
install("src/wizard-0.2.6.html","rootfs-overlay/usr/share/2pny/wizard.html",0o644)
install("src/dashboard-0.2.6.html","rootfs-overlay/usr/share/2pny/dashboard.html",0o644)
install("src/2pny-mmdvmhost-0.2.6.service","rootfs-overlay/etc/systemd/system/2pny-mmdvmhost.service",0o644)
install("src/2pny-dmrgateway-0.2.6.service","rootfs-overlay/etc/systemd/system/2pny-dmrgateway.service",0o644)
install("src/2pny-display-0.2.6.service","rootfs-overlay/etc/systemd/system/2pny-display.service",0o644)
install("src/mosquitto-pu2pny-0.2.6.conf","rootfs-overlay/etc/mosquitto/conf.d/pu2pny-local.conf",0o644)
...
if anchor not in s:
    raise SystemExit("0.2.6 builder: MMDVM build purge anchor not found")
if "PU2PNY_DMRGATEWAY_DISPLAY_0_2_6" not in s:
    block=r'''# PU2PNY_DMRGATEWAY_DISPLAY_0_2_6
...
runtime_new="apt-get install -y --no-install-recommends libmosquitto1 mosquitto"
if runtime_old not in s:
    raise SystemExit("0.2.6 builder: runtime mosquitto anchor not found")
...
assert re.search(r'appVersion\s*=\s*"0\.2\.6-alpha"',main)
...
for needle in ("RX · Rádio → hotspot","TX · Rede → rádio","Internet e rede local","MTR / qualidade"):
    assert needle in dash
assert DMRGATEWAY_COMMIT in builder.read_text()
assert DISPLAY_COMMIT in builder.read_text()
print("PU2PNY-OS 0.2.6 radio/network/display/live patch applied")
