    ...
    if unit=="2pny-dstargateway.service":
        ...
        if m:
            ...
            runtime.update({"protocol":"DSTAR","last_command":"link","last_command_target":target,
                            ...
                            "link_state":"linking"})
            if len(target)>=2 and target[-1].isalpha():
                runtime["module"]=target[-1]
                runtime["server_name"]=target[:-1].strip()
            changed=True
        ...
        if m:
            ...
            changed=True
        if re.search(r"link (?:to .* )?(?:failed|refused)|not linked|unlink(?:ed|ing)?",msg,re.I):
            # Keep an explicit linking command visible until the gateway
            # provides the actual result, but never call it connected.
            if not re.search(r"Link command",msg,re.I):
                runtime.update({"protocol":"DSTAR","linked":False,"connected":False,"link_state":"unlinked"});changed=True
