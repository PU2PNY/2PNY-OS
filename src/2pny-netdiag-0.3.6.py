#!/usr/bin/env python3
"""PU2PNY-OS 0.3.6 lightweight network diagnostics.
...
def effective_dns(iface):
    """Return only DNS resolvers effective for the default-route interface."""
    out=[]
    if iface:
...
def classify(metrics):
    if not metrics.get("reachable"): return "terrible","Péssimo"
    loss=float(metrics.get("loss_pct") or 0); lat=float(metrics.get("latency_ms") or 0); jit=float(metrics.get("jitter_ms") or 0)
    if loss>15 or lat>350 or jit>100:return "terrible","Péssimo"
    if loss>5 or lat>250 or jit>60:return "poor","Ruim"
    if loss>1 or lat>140 or jit>35:return "good","Bom"
    return "best","Melhor opção"
