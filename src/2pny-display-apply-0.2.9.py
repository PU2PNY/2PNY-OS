...
elif d.get("state")=="mmdvm_display_candidate" and m.get("detected"):
    # A MMDVM serial display port is a safe candidate to TRY after RF is
    # configured. We do not claim protocol confirmation until the display
    # service is actually healthy.
    enabled=True; kind="nextion_mmdvm"; port="modem"; confidence="auto_candidate"
...
if enabled and kind=="nextion_mmdvm" and not port:
    port="modem"
...
if (STATE/"rf-configured").exists() and service("is-active","--quiet","2pny-mmdvmhost.service"):
    ...
    if active:
        message="MMDVM-Display ativo. Nextion via MMDVM habilitada automaticamente." if confidence=="auto_candidate" else "MMDVM-Display ativo e conectado à Nextion."
    else:
        message="Tentativa automática da Nextion falhou; display deixado para confirmação manual." if confidence=="auto_candidate" else "MMDVM-Display não permaneceu ativo; verifique layout/porta."
        if confidence=="auto_candidate":
            service("disable","--now",SERVICE)
