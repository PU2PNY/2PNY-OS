...
"""
    ...
    xlx_hosts=""
    if kind.lower()=="tgif":
        # PROTO-019: use the TGIF rewrite contract documented by the exact
        # pinned DMRGateway revision. TGIF carries the network side on TS2;
        # map the locally selected RF slot to/from that network slot.
        rewrite_lines=[]
        for idx,s in enumerate((1,2) if slot=="both" else (int(slot),)):
            rewrite_lines += [
...
try:
    ...
    if not service_active(HOST_SERVICE):
        ...
        raise RuntimeError("MMDVMHost did not remain active with DMRGateway"+(": "+detail if detail else ""))
