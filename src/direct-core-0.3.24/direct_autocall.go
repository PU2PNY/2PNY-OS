		...
		if err==nil && st.ModTime()!=lastMod {
			...
			if b,e:=os.ReadFile(livePath);e==nil&&json.Unmarshal(b,&l)==nil&&l.Active!=nil {
				...
				if sig!=lastSig {
					...
					status:=c.st.Status
					currentPeer:=c.st.Peer
					idle:=!c.st.RadioActive&&(status==""||status=="idle")
					c.mu.Unlock()
					to:=resolveAutoPeer(peers,l,ct)
					if to!=""&&strings.EqualFold(currentProtocol(),l.Active.Protocol) {
						if status=="incoming"&&strings.EqualFold(currentPeer,to) {
							go func(target string){ _=c.acceptIncoming(target) }(to)
						} else if idle {
							c.mu.Lock()
