...
func (c *core) handlePacket(raw []byte, addr *net.UDPAddr) {
	...
	if json.Unmarshal(raw, &m) == nil && m.T != "" {
		...
		case "relay-data":
			if addr == nil || c.serverAddr == nil || addr.Port != c.serverAddr.Port || !addr.IP.Equal(c.serverAddr.IP) {
				return
			}
			blob, e := unb64(m.Packet)
			if e != nil || len(blob) == 0 || len(blob) > 65535 {
				return
			}
			c.handleSecure(blob, addr, true)
			return
		}
		return
	}
...
func (c *core) handleSecure(raw []byte, addr *net.UDPAddr, relayed bool) {
	...
	case "probe":
		...
		if _, ok := directRadioProfiles[proto]; !ok {
			return
		}
		path := "Direct"
		traversal := "UDP hole punching"
		if relayed {
			path = "Relay"
			traversal = "Encrypted relay fallback"
		}
		c.mu.Lock()
		if c.st.RadioActive && c.st.Peer != pkt.From {
			c.mu.Unlock()
			return
		}
		c.st.Status = "incoming"
		c.st.Peer = pkt.From
		c.st.Path = path
		c.st.Protocol = proto
		c.st.InitiatedBy = "remote"
		c.st.Traversal = traversal
		c.st.LastError = ""
		c.incomingPeer = pkt.From
		c.incomingRelayed = relayed
		c.incomingProbe = append([]byte(nil), plain...)
		c.incomingAt = time.Now()
		c.lastPeerTraffic = time.Now()
		c.writeState()
		c.mu.Unlock()
		// Do not stop the currently active gateway here. The radio operator
		// accepts the call by addressing this same peer; only then enterRadio.
		_ = c.sendSecure(pkt.From, "ringing", []byte("waiting-radio-accept"), relayed)
	case "ringing":
		c.mu.Lock()
		ch := c.ringWait[pkt.From]
		c.mu.Unlock()
		if ch != nil {
			select {
			case ch <- true:
