...
func (c *core) sendSecure(to, kind string, payload []byte, useRelay bool) error {
	...
	c.mu.Unlock()
	if ep == nil {
		return errors.New("endpoint ponto a ponto ainda não negociado")
	}
	if _, e = c.conn.WriteToUDP(raw, ep); e != nil {
		return fmt.Errorf("envio P2P falhou: %w", e)
	}
	return nil
}

func (c *core) acceptIncoming(to string) error {
	to = strings.ToUpper(strings.TrimSpace(to))
	c.mu.Lock()
	if c.st.Status != "incoming" || c.incomingPeer != to || len(c.incomingProbe) == 0 {
		c.mu.Unlock()
		return errors.New("não há chamada pendente deste contato")
	}
	proto := c.st.Protocol
	relay := c.incomingRelayed
	probe := append([]byte(nil), c.incomingProbe...)
	c.mu.Unlock()
	...
	c.st.InitiatedBy = "accepted-by-radio"
	c.st.Traversal = map[bool]string{true: "Encrypted relay fallback", false: "UDP hole punching"}[relay]
	c.st.LastError = ""
	c.lastPeerTraffic = time.Now()
	c.writeState()
	c.mu.Unlock()
	if err := c.sendSecure(to, "probe-ack", probe, relay); err != nil {
		c.exitRadio()
		return err
	}
	return nil
...
func (c *core) call(to string) error {
	...
	c.st.Peer = to
	c.st.Path = "Connecting"
	c.st.Traversal = "UDP hole punching"
	c.st.RequestedTarget = to
	c.st.LastError = ""
	...
	defer func() {
		c.mu.Lock()
		delete(c.probeWait, to)
		delete(c.ringWait, to)
		c.mu.Unlock()
	}()

	probePayload := []byte(fmt.Sprintf("%d", time.Now().UnixNano()))
	...
	chosenRelay := false
	gotRing := false
	for _, relay := range paths {
		c.mu.Lock()
		if relay {
			c.st.Path = "Relay"
			c.st.Traversal = "Encrypted relay fallback"
		} else {
			c.st.Path = "Connecting"
			c.st.Traversal = "UDP hole punching"
		}
		c.writeState()
		c.mu.Unlock()
		if err := c.sendSecure(to, "probe", probePayload, relay); err != nil {
			if !relay {
				continue
			}
			return err
	...
	if !gotRing {
		...
		c.st.Status = "error"
		c.st.Path = "Offline"
		c.st.LastError = "o peer não respondeu nem por caminho direto nem pelo relay criptografado"
		c.writeState()
		c.mu.Unlock()
		return errors.New("PU2PNY Direct: peer sem resposta")
	}
	...
	case d := <-ack:
		if err := c.enterRadio(localProto); err != nil {
			_ = c.sendSecure(to, "hangup", []byte("radio-setup-failed"), chosenRelay)
			c.mu.Lock()
			c.st.Status = "error"
			c.st.Path = "Offline"
	...
	case <-time.After(30 * time.Second):
		_ = c.sendSecure(to, "hangup", []byte("accept-timeout"), chosenRelay)
		c.mu.Lock()
		c.st.Status = "idle"
		c.st.Peer = ""
		c.st.Path = "Offline"
		c.st.LastError = "chamada não aceita pelo rádio dentro do limite"
		c.writeState()
		c.mu.Unlock()
		return errors.New("chamada não aceita pelo rádio")
	}
}
