...
func newCore(dir, callsign, server string) (*core, error) {
	...
	c := &core{dir: dir, server: server, id: id, edPriv: priv, edPub: pub, xPriv: xk, conn: conn, serverAddr: ra, peers: loadPeers(dir), peerEndpoints: map[string]*net.UDPAddr{}, lastSeq: map[string]uint64{}, pending: map[string]chan ctrl{}, probeWait: map[string]chan time.Duration{}, ringWait: map[string]chan bool{}}
