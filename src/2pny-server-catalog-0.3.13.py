...
def decorate(x):
    ...
    if kind=="BrandMeister":
        x.update(password_label="Senha de segurança do hotspot",password_required=True,api_supported=True,essid_supported=True)
    elif kind=="XLX":
        ...
        x.setdefault("password_label","Chave de autenticação DAPNET");x["password_required"]=True
    elif kind=="TGIF":
        x.setdefault("essid_supported",True);x.setdefault("password_required",False);x.setdefault("password_label","Chave de segurança TGIF")
    elif kind in ("FreeDMR","DMR+","HBLink","DMR"):
