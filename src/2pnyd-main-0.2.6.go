...
func displayOverrideHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		if b, err := os.ReadFile(displayOverrideFile); err == nil {
			var ov map[string]any
			if json.Unmarshal(b,&ov)==nil { if n,ok:=ov["layout"].(float64); ok { layout=int(n) } }
		}
	...
	case http.MethodPost:
		...
		if in.Enabled {
			...
			if in.Layout != 0 && in.Layout != 2 && in.Layout != 3 {
				writeJSON(w, http.StatusBadRequest, map[string]any{"ok":false,"error":"layout Nextion deve ser 0, 2 ou 3"})
				return
			}
...
func hardwareScanHandler(w http.ResponseWriter, r *http.Request) {
	...
	_ = exec.Command("/usr/local/sbin/2pny-display-status", "hardware", "Preparando drivers").Run()
	go func() {
...
func basicApplyHandler(w http.ResponseWriter, r *http.Request) {
	...
	if in.Protocol == "DMR" {
		...
		if in.XLXModule != "" && !regexp.MustCompile(`^[A-Z]package main
