...
# DISPLAY-020: 0.3.20 standardizes modem-attached Nextion on PU2PNY Moderno V2.
# A real COMOK obtained through MMDVMHost's MQTT serial bridge is required before
# the runtime may claim physical confirmation. MMDVMHost remains UART owner.
if kind=="nextion_mmdvm":
    if d.get("physical_confirmed") is not True:
        write_json(STATUS,{"state":"not_confirmed","active":False,"type":kind,
                           "renderer":"pu2pny-modern-v2","writer":"none",
                           "message":"Nextion candidata, mas a comunicação física COMOK ainda não foi confirmada.",
                           "updated":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime())},0o644)
        print("DISPLAY_NOT_CONFIRMED");raise SystemExit(0)
