...
def restore():
    if (backup/"MMDVM-Host.ini").exists():shutil.copy2(backup/"MMDVM-Host.ini",HOST)
...
"""
        ...
        if p.exists():shutil.copy2(p,d/"P25Hosts.json")
...
"""
        ...
        if p.exists():shutil.copy2(p,d/"NXDNHosts.json")
        ident=re.search(r"\d{2,7}",server);static=ident.group(0) if ident else ""
        ini=f"""[General]
Callsign={callsign}
Suffix=NXDN
...
"""
    ...
    time.sleep(1)
    if not active(unit):raise RuntimeError(unit+" did not remain active")
    if not ctl("restart","2pny-mmdvmhost.service"):raise RuntimeError("MMDVMHost failed")
    time.sleep(2)
    if not active("2pny-mmdvmhost.service"):raise RuntimeError("MMDVMHost did not remain active")
