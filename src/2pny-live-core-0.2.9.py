...
DSTAR_START_RE = re.compile(
    r"D-Star, received (RF|network) (?:header|late entry) from\s+([^/\s,]+)(?:/([^\s,]+))?\s+to\s+([^,]+?)(?:\s+via\s+([^,]+))?$", re.I)
DSTAR_END_RE = re.compile(
    r"D-Star, (?:received )?(RF|network) (?:end of transmission|transmission lost).*?from\s+([^/\s,]+)(?:/([^\s,]+))?\s+to\s+([^,]+)", re.I)
...
class LiveState:
    ...
    def _metric(self, protocol, slot=None, ber=None, rssi=None):
        ...
        for item in self.active.values():
            if not item or item.get("protocol")!=protocol:
                continue
            if slot is not None and item.get("slot") not in (None,slot):
                continue
            current=item;break
        if not current:
            return None
    ...
    def ingest_json(self, payload, ts=None):
        ...
        for key,proto in (("DMR","DMR"),("D-Star","DSTAR"),("YSF","YSF"),("P25","P25"),("NXDN","NXDN")):
            ...
            direction="RF" if str(j.get("source") or "").lower()=="rf" else "NETWORK"
            if action in ("start","late_entry"):
                if proto=="DMR":
                    ...
                    return self._start(proto,direction,src,target,ts,slot=int(j.get("slot") or 0) or None,
            ...
            if action in ("end","lost"):
                return self._finish(direction,ts,reason=action,protocol=proto,
