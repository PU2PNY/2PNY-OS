...
func wifiScanHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		writeJSON(w, http.StatusServiceUnavailable, map[string]any{"ok":false, "error":msg, "networks":[]any{}})
		return
	}
	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	w.Header().Set("Cache-Control", "no-store")
...
func networkConnectStatusHandler(w http.ResponseWriter, r *http.Request) {
	b, err := os.ReadFile(filepath.Join(dataDir, "network-connect.json"))
	if err != nil {
...
func networkConnectHandler(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusAccepted, map[string]any{
		...
		"keep_ap":keep,
		"message":"Dados recebidos. A conexão Wi-Fi será feita em seguida.",
		"reconnect_url":"http://pu2pny.local",
	})
}
