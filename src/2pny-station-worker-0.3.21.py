...
def update_network_runtime_from_gateway(msg, ts=None, unit=""):
    ...
    if m:
        tg=int(m.group(1))
        runtime.update({"protocol":"DMR","requested_module":m.group(2).upper(),"requested_module_tg":tg,
                        "requested_tg":tg,"module_source":"rf-control","link_state":"linking"})
        changed=True
    ...
    if m:
        ...
        tg=int(runtime.get("requested_tg") or runtime.get("requested_module_tg") or (4000+(ord(letter)-64)))
        runtime.update({"protocol":"DMR","server_name":"XLX"+m.group(1),"module":letter,
                        "module_tg":4000+(ord(letter)-64),"connected_tg":tg,"requested_tg":"",
                        "module_source":"gateway","linked":True,"connected":True})
        changed=True
    if "PU2PNY, XLX module control: unlinked by TG4000" in msg:
        runtime.update({"protocol":"DMR","module":"","module_tg":4000,"connected_tg":4000,"requested_tg":"",
                        "module_source":"rf-control","linked":False,"connected":False})
        changed=True
...
def persist_end(db,event):
    ...
    event=dict(event)
    if str(event.get("protocol") or "").upper()=="DMR":
        rt=read_json(NETWORK_RUNTIME,{})
        if not event.get("module") and rt.get("module"):
            event["module"]=rt.get("module")
            event["module_tg"]=rt.get("module_tg")
        if rt.get("connected_tg"):
            event["connected_tg"]=rt.get("connected_tg")
