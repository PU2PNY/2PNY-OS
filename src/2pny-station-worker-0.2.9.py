#!/usr/bin/env python3
"""PU2PNY resident station worker 0.2.9.
...
def fetch(url,data=None,limit=2*1024*1024):
    req=urllib.request.Request(url,data=data,headers={"User-Agent":"PU2PNY-OS/0.2.9"})
...
def qrz_photo(call):
    ...
    if cfg.get("username") and cfg.get("password"):
        ...
        auth=xml({"username":cfg["username"],"password":cfg["password"],"agent":"PU2PNY-OS-0.2.9"})
        if auth.get("Key"):
...
def main():
    ...
    hostlog=start_follow("2pny-mmdvmhost.service");gwlog=start_follow("2pny-dmrgateway.service");mqtt=start_mqtt()
    sel=selectors.DefaultSelector()
    sel.register(hostlog.stdout,selectors.EVENT_READ,("journal",False))
    sel.register(gwlog.stdout,selectors.EVENT_READ,("journal",True))
    sel.register(mqtt.stdout,selectors.EVENT_READ,("mqtt",False))
    ...
    last_contacts=0;last_housekeeping=0;last_telemetry=0;previous_cpu=None
