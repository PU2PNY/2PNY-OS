...
def radio_active():
    return os.system("systemctl is-active --quiet 2pny-mmdvmhost.service >/dev/null 2>&1") == 0

def main():
    state = sys.argv[1] if len(sys.argv) > 1 else "boot"
    message = sys.argv[2] if len(sys.argv) > 2 else PROGRESS.get(state, (0, "PU2PNY"))[1]
    if radio_active() and state != "error":
        write_state(state, message, False, "mmdvmhost-active")
        return 0
