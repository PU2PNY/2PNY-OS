...
BASE=STATE/"updates";CACHE=BASE/"cache";BACKUPS=STATE/"backups/app-update";STATUS=BASE/"status.json";STAGED=BASE/"staged.json";LOCK=BASE/"update.lock";VERSION_FILE=Path("/etc/2pny/version")
...
def restore_backup_file(backup,created,old_version):
    if backup and backup.exists():
        r=run("tar","-C","/","-xzf",str(backup),timeout=120)
        if r.returncode:return False,"falha ao extrair o ponto de retorno"
    for dst in created:
        try:dst.unlink(missing_ok=True)
        except Exception:pass
    try:
        if old_version is None:VERSION_FILE.unlink(missing_ok=True)
        else:
            VERSION_FILE.parent.mkdir(parents=True,exist_ok=True)
            VERSION_FILE.write_text(old_version)
    except Exception as e:return False,"falha ao restaurar a versão: "+str(e)
    run("systemctl","daemon-reload",timeout=20)
    for u in ("2pny-station.service","2pny-display-core.service","2pny-netdiag.service","2pnyd.service"):
        run("systemctl","try-restart",u,timeout=20)
    return True,"estado anterior restaurado automaticamente"
...
def install_staged(digest,version,keep):
    ...
    backup=None;mutated=False;created=[];old_version=None
    try:
        try:old_version=VERSION_FILE.read_text()
        except FileNotFoundError:old_version=None
        ...
        if VERSION_FILE.exists():existing.append(str(VERSION_FILE).lstrip("/"))
        existing=list(dict.fromkeys(existing))
        created=[dst for _,dst in targets if not dst.exists()]
        if existing:
        ...
        if mutated:
            ok,msg=restore_backup_file(backup,created,old_version)
            atomic_status(state="rolled_back" if ok else "error",
                          message=("Instalação falhou; "+msg+".") if ok else ("Instalação falhou e o rollback automático também falhou: "+msg),
                          version=version,progress_percent=100,backup=backup.name if backup and backup.exists() else "",verified=True)
        raise
    finally:
