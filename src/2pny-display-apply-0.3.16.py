...
# DISPLAY-018: physical validation showed the MQTT/vector bridge regressed
# modem-attached Nextion. Restore the 0.3.8 proven writer: MMDVMHost itself.
# Direct Nextion/OLED/LCD below remain on PU2PNY Display Core.
if kind=="nextion_mmdvm":
    renderer="mmdvmhost-native"
    if requested not in (2,3):requested=2
...
settings.update({"enabled":True,"driver":"pu2pny-display-core","renderer":"pu2pny-modern-v2","writer":"pu2pny-display-core",
                 "type":kind,"automatic":True,"model_profile":profile,"resolution":resolution,
                 "requested_layout":9,"effective_layout":9})
write_json(SETTINGS,settings)
ctl("enable",CORE)
started=False
...
write_json(STATUS,{"state":"active" if started else "configured","active":started,"type":kind,
                   ...
                   "model_profile":profile,"resolution":resolution,"automatic":True,"message":msg,
                   "updated":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime())},0o644)
