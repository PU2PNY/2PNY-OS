(function(){
  ...
  function ensureTools(active){
    ...
    getj('/api/language',null,3500).then(function(v){var lang=v&&v.language;if(/^(pt|en|es)$/.test(lang||'')){localStorage.setItem('pu2pny-language',lang);if(q('language'))q('language').value=lang;if(window.translateNavigation)window.translateNavigation()}}).catch(function(){})
    if(!q('modeToggle')){
  ...
  var clockStarted=false,clockServerUTC=0,clockSyncAt=0,clockTZ='',clockDSTManual=false;
  ...
  async function syncClock(){
    try{var s=await getj('/api/system',null,4500),utc=Date.parse(s.utc||'');if(Number.isFinite(utc)){clockServerUTC=utc;clockSyncAt=Date.now()}clockTZ=s.timezone||clockTZ;clockDSTManual=!!(s.clock_settings&&s.clock_settings.dst_manual)}catch(e){}
    renderClock()
  }
  ...
  window.PNY={q:q,esc:esc,getj:getj,setTheme:setTheme,initTheme:initTheme,flagUrl:flagUrl,fmtTime:fmtTime,nav:nav,startClock:startClock,operation:operation,realOnly:realOnly,sweepRealOnly:sweepRealOnly};
