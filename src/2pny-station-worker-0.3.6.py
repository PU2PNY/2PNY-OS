    ...
    if unit=="2pny-dstargateway.service":
        ...
        if m:
            ...
            runtime.update({"protocol":"DSTAR","server_name":server,"module":letter,
                            "linked":True,"connected":True,"module_source":"gateway"})
            changed=True
        if re.search(r"link (?:to .* )?(?:failed|refused)|not linked|unlink",msg,re.I):
            runtime.update({"protocol":"DSTAR","linked":False,"connected":False});changed=True
