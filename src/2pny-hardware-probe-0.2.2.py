#!/usr/bin/env python3
"""PU2PNY hardware discovery probe.
...
if not state["display"].get("detected") and state["mmdvm"].get("detected"):
    desc = str(state["mmdvm"].get("description", "")).lower()
    profile = str(state["mmdvm"].get("profile", "")).lower()
    if any(word in (desc + " " + profile) for word in ("mmdvm_hs", "mmdvm hs", "zumspot", "hat")):
        state["display"] = {
            "detected": True,
            "state": "mmdvm_display_port",
            "class": "nextion_mmdvm",
            "model": "Nextion via MMDVM",
            "port": "modem",
            "confidence": "mmdvm_interface",
            "message": "Interface de display da MMDVM reconhecida e preparada para Nextion.",
        }
    else:
        state["display"] = {
            "detected": False,
