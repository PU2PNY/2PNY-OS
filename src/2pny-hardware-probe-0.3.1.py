#!/usr/bin/env python3
"""PU2PNY hardware discovery probe 0.3.1.
...
if not state["display"].get("detected") and state["mmdvm"].get("detected"):
    bridge = probe_nextion_mmdvm(state["mmdvm"].get("port", ""), state["mmdvm"].get("baud", 115200))
    if bridge:
        state["display"] = bridge
    else:
        state["display"] = {
            "detected": False,
            ...
            "message": "Canal de display da MMDVM disponível; a Nextion será testada novamente automaticamente após o rádio iniciar.",
        }
