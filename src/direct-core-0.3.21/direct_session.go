...
func (c *core) sendSecure(to, kind string, payload []byte, forceRelay bool) error {
	if forceRelay {
		return errors.New("relay desativado: PU2PNY Direct aceita somente caminho ponto a ponto")
	}
	raw, e := c.encrypt(to, kind, payload)
	if e != nil { return e }
	c.mu.Lock(); ep := c.peerEndpoints[to]; c.mu.Unlock()
	if ep == nil { return errors.New("endpoint ponto a ponto ainda não negociado") }
	if _, e = c.conn.WriteToUDP(raw, ep); e != nil { return fmt.Errorf("envio P2P falhou: %w", e) }
	return nil
}
...
func (c *core) call(to string) error {
	...
	c.st.Path = "Blocked"
	c.st.Traversal = "UDP hole punching"
	c.st.LastError = "caminho P2P direto não foi aberto; CGNAT/NAT restritivo pode impedir a conexão. O PU2PNY não encaminha o QSO pelo servidor."
	c.writeState()
	c.mu.Unlock()
	return errors.New("caminho P2P direto indisponível; relay de QSO desativado")}
