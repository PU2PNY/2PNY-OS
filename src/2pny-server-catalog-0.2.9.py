#!/usr/bin/env python3
"""PU2PNY unified protocol catalog 0.2.9.
...
def read_dstar():
    ...
    for filename,kind,prefix in (("DPlus_Hosts.txt","DPlus","REF"),("DExtra_Hosts.txt","DExtra","XRF"),("DCS_Hosts.txt","DCS","DCS")):
        ...
        for raw in p.read_text(errors="ignore").splitlines():
            ...
            out.append({"name":name.upper(),"address":address,"port":0,"kind":kind,"description":f"{prefix} {name}","modules":AZ,"default_module":"C","module_supported":True})
    p=BASE/"XLXHosts.txt"
    if p.exists():
        for raw in p.read_text(errors="ignore").splitlines():
            parts=raw.strip().split(";")
            ...
            if re.fullmatch(r"[0-9A-Z]{3}",ident):
                out.append({"name":"XLX"+ident,"id":ident,"address":address,"port":0,"kind":"XLX","description":"XLX "+ident,"modules":AZ,"default_module":"D" if ident=="026" else "A","module_supported":True})
    return out
