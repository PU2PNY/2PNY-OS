(function(){
  ...
  function ensureGlobalCSS(){
    ...
    st.textContent='.topnav{min-height:64px;height:auto}.topnav .nav{min-width:0;overflow-x:auto;flex-wrap:nowrap;scrollbar-width:thin}.topnav .nav a{white-space:nowrap;flex:0 0 auto}.navtools{flex:0 0 auto;white-space:nowrap}.navtools .pill,#clockLocal{white-space:nowrap;min-width:76px}.pny-health{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin:12px 0}.pny-health .card{padding:12px}.pny-meter{height:10px;border-radius:999px;background:var(--line);overflow:hidden;margin-top:7px}.pny-meter i{display:block;height:100%;background:var(--blue);width:0}.pny-profilebar{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}.pny-profilebar button.active{outline:2px solid var(--green)}.pny-protocol-frame{width:100%;height:980px;border:0;border-radius:14px;background:var(--card)}@media(max-width:760px){.pny-health{grid-template-columns:1fr}.topnav{align-items:flex-start}.navtools{margin-left:auto}.pny-protocol-frame{height:1200px}}';
  ...
  function nav(active){var items=[['/dashboard','Ao Vivo','live'],['/internet','Internet','internet'],['/hotspot','Hotspot / Protocolos','hotspot'],['/direct','Direct','direct'],['/aprs','APRS / D-PRS','aprs'],['/history','Histórico','history'],['/display','Display','display'],['/system','Sistema','system']];var n=q('mainNav');if(n)n.innerHTML=items.map(function(x){return '<a '+(x[2]===active?'class="active" ':'')+'href="'+x[0]+'">'+x[1]+'</a>'}).join('');ensureTools(active);startClock();startAPRSWatch();setTimeout(enhanceCurrentPage,0)}
  ...
  function startClock(){if(clockStarted)return;clockStarted=true;clock();setInterval(clock,1000)}
  ...
  function ensureHealth(){
    ...
    var sec=document.createElement('section');sec.id='pnyHealth';sec.className='section';
    sec.innerHTML='<div class="sectionhead"><h2>Saúde da comunicação</h2><span class="muted">RF · uplink · Internet</span></div><div class="pny-health"><article class="card" id="pnyRFHealth"><small class="muted">Sinal RF</small><div class="big" id="pnyRFValue">—</div><div class="pny-meter"><i id="pnyRFBar"></i></div><small id="pnyRFMeta" class="muted">Aguardando RX do rádio</small></article><article class="card" id="pnyWiFiHealth" role="button" tabindex="0"><small class="muted">Wi-Fi / Uplink</small><div class="big" id="pnyWiFiValue">—</div><div class="pny-meter"><i id="pnyWiFiBar"></i></div><small id="pnyWiFiMeta" class="muted">—</small></article><article class="card" id="pnyNetHealth" role="button" tabindex="0"><small class="muted">Internet</small><div class="big" id="pnyNetValue">—</div><div class="pny-meter"><i id="pnyNetBar"></i></div><small id="pnyNetMeta" class="muted">—</small></article></div><div class="pny-profilebar" id="pnyProfiles"></div>';
  ...
  function startHealthStream(){
    ...
    try{pnyLiveES=new EventSource('/api/live/events');pnyLiveES.onmessage=function(e){try{applyLiveHealth(JSON.parse(e.data))}catch(_){}};pnyLiveES.onerror=function(){}}catch(e){}
  }
  async function loadProfiles(){
    ...
    try{
      ...
      q('pnyProfiles').querySelectorAll('button').forEach(function(b){b.onclick=async function(){var p=b.dataset.p;if(b.dataset.ok!=='1'){location.href='/hotspot#protocols';return}var op=operation('Ativando '+p,'Aplicando o perfil de forma transacional...');try{await getj('/api/protocol/profiles',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'activate',protocol:p})},70000);op.done('Perfil ativado.');setTimeout(loadProfiles,900)}catch(e){op.error(e.message)}}})
    }catch(e){}
  }
  function enhanceHotspot(){
    if(location.pathname!='/hotspot'||q('pnyProtocolsUnified'))return;var main=document.querySelector('main');if(!main)return;
    var sec=document.createElement('section');sec.id='pnyProtocolsUnified';sec.className='section';
    sec.innerHTML='<div class="sectionhead" id="protocols"><h2>Protocolos</h2><span class="muted">Configuração integrada ao Hotspot</span></div><div class="card" style="padding:0;overflow:hidden"><iframe class="pny-protocol-frame" title="Configuração de protocolos" src="/protocols/embed"></iframe></div>';
