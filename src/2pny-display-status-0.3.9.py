...
PROGRESS = {
    "boot": (10, "Iniciando / Starting"),
    "network": (25, "Rede / Network"),
    "maintenance": (0, "Manutencao / Maintenance"),
    "hardware": (55, "Hardware / Hardware"),
    "display": (68, "Tela / Display"),
    "rf": (82, "Radio / Radio"),
    "ready": (100, "Pronto / Ready"),
    "warning": (70, "Atencao / Warning"),
    "error": (0, "Erro / Error"),
}
...
def screen_commands(state, message):
    ...
    cmds=[
        "bkcmd=0","dim=80","cls 0","draw 12,12,307,218,33808",
        "xstr 22,24,276,34,0,65535,0,1,1,1,\"PU2PNY\"",
        f"xstr 22,72,276,28,0,{accent},0,1,1,1,\"{safe(title)}\"",
        f"xstr 22,154,276,24,0,65535,0,1,1,1,\"{msg}\"",
    ]
    # Maintenance/update has no measurable package percentage. Never display a
    # fabricated bar that can appear frozen.
    if state=="maintenance":
        cmds += ["xstr 22,118,276,24,0,2016,0,1,1,1,\"Em andamento / Working\""]
    else:
        cmds += ["draw 22,124,297,146,65535",f"fill 23,125,{fillw},20,{accent}",
...
def radio_or_display_active():
    return service_active("2pny-mmdvmhost.service") or service_active("2pny-display-core.service") or service_active("2pny-display.service")
...
def main():
    ...
    # A Nextion connected through the MMDVM belongs exclusively to
    # MMDVMHost. Drawing a temporary full-screen canvas here can leave an
    # unknown HMI stuck on "RF/maintenance" after MMDVMHost takes ownership.
    # Persist the state for the web/runtime, but never become a second writer.
    if d.get("class") == "nextion_mmdvm":
        write_state(state, message, False, "mmdvmhost-authoritative")
        return 0
