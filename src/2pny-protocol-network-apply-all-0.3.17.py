#!/usr/bin/env python3
"""Apply PU2PNY 0.3.10 protocol/gateway configuration transactionally.
...
# PROTO-024: Module is the LOCAL RF module, not the remote reflector room.
# The PU2PNY hotspot loopback is C on both MMDVMHost and DStarGateway.
# The operator-selected A-Z module remains only in the remote reflector target.
setsec(cp,"D-Star",{"Enable":"1" if proto=="DSTAR" else "0","Module":"C" if proto=="DSTAR" else cp.get("D-Star","Module",fallback="C")})
...
try:
    ...
    if proto=="DSTAR":
        # The image pins F4FXL DStarGateway v20260323-612f388. PROTO-024
        # keeps the local HB repeater identity fixed at module C while the
        # selected reflector/module remains a remote link target. The native
        ...
        dpath=str(d)+"/";cpath=str(custom)+"/";audio_path="/usr/share/2pny/audio/dstar/"
        ini=f"""[General]
Callsign={callsign}
Address=0.0.0.0
HBAddress=127.0.0.1
...
"""
        ...
        # Ensure the server selected in the panel exists under exactly the
        # name YSFGateway will search.  This is important for project presets.
        ...
        if kind.upper()=="YSF" and address and port>0:
            ...
            for x in rows:
                ...
                nm=str(x.get("name") or "");co=str(x.get("country") or "")
                fullnames.append(("XX-"+nm) if x.get("use_xx_prefix") else (co+"-"+nm))
            if server not in fullnames:
                if "-" in server:co,nm=server.split("-",1)
                else:co,nm="YSF",server
                digits="".join(re.findall(r"\d",server))[-5:] or "00000"
                rows.append({"designator":digits.zfill(5),"country":co,"name":nm,"use_xx_prefix":False,
...
atomic_json(STATEFILE,{
 ...
 "module":module if proto=="DSTAR" else "","state":"gateway_active","connected":False,"gateway":SERVICES[proto],
 "updated":datetime.datetime.now(datetime.timezone.utc).isoformat()
})
