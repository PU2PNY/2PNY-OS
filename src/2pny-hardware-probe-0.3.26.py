#!/usr/bin/env python3
"""PU2PNY hardware discovery probe 0.3.26.
...
SERIAL_LOCK = Path("/run/2pny/mmdvm-serial.lock")
SERIAL_LOCK.parent.mkdir(parents=True, exist_ok=True)
# HW-003/RF-017: every identification pass owns the same lock used by RF apply.
# The descriptor intentionally stays open until process exit.
_serial_lock = SERIAL_LOCK.open("a+")
fcntl.flock(_serial_lock.fileno(), fcntl.LOCK_EX)
...
def parse_mmdvm(raw):
    for pos, value in enumerate(raw):
        ...
        if "mmdvm_hs" in low or "mmdvm hs" in low:
            ...
            profile = "DVMega compatível"
        elif "n5boc" in low:
            profile = "N5BOC MMDVM compatível"
        elif "repeater" in low and "mmdvm" in low:
            profile = "MMDVM Repeater compatível"
...
def probe_mmdvm(dev, driver=""):
    ...
    for baud in baud_order:
        ...
        try:
            ...
            if not result:
                os.write(fd, request)
                result = parse_mmdvm(read_for(fd, 0.22))
            if result:
                result["baud"] = baud
                return result, None
        except Exception as exc:
...
state = {
    ...
    "mmdvm": {"detected": False, "duplex": "unknown", "capabilities": [], "transports_checked": ["uart"]},
