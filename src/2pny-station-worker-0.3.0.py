#!/usr/bin/env python3
"""PU2PNY resident station worker 0.3.0.
...
def fetch(url,data=None,limit=2*1024*1024):
    req=urllib.request.Request(url,data=data,headers={"User-Agent":"PU2PNY-OS/0.3.0"})
...
def qrz_photo(call):
    ...
    if cfg.get("username") and cfg.get("password"):
        ...
        auth=xml({"username":cfg["username"],"password":cfg["password"],"agent":"PU2PNY-OS-0.3.0"})
        if auth.get("Key"):
