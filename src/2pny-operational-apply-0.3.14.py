#!/usr/bin/env python3
import configparser,fcntl,json,os,subprocess,sys,time
from pathlib import Path
STATE=Path("/var/lib/2pny");RUN=Path("/run/2pny");CFG=STATE/"config.json";RF=STATE/"rf-configured"
DISABLED=STATE/"operational-disabled";REQ=RUN/"operational-request.json";RES=RUN/"operational-result.json";LOCK=RUN/"operational.lock"
HOST="2pny-mmdvmhost.service";HOSTCFG=STATE/"mmdvm/MMDVM-Host.ini"
RESTORING=False
GATEWAYS={"DMR":"2pny-dmrgateway.service","DSTAR":"2pny-dstargateway.service","YSF":"2pny-ysfgateway.service","P25":"2pny-p25gateway.service","NXDN":"2pny-nxdngateway.service","POCSAG":"2pny-dapnetgateway.service"}
def run(*a,timeout=20):return subprocess.run(list(a),text=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=timeout)
def active(u):return run("systemctl","is-active","--quiet",u,timeout=5).returncode==0
def result(o):
 RUN.mkdir(parents=True,exist_ok=True);o=dict(o);o.setdefault("updated",time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()))
 t=RES.with_suffix(".tmp");t.write_text(json.dumps(o,ensure_ascii=False)+"\n");os.chmod(t,0o644);os.replace(t,RES)
 if RESTORING:
  p=STATE/"last-boot-restore.json";tmp=p.with_suffix(".tmp");tmp.write_text(json.dumps(o,ensure_ascii=False,indent=2)+"\n");os.chmod(tmp,0o600);os.replace(tmp,p)
def journal(u):
 try:return run("journalctl","-u",u,"-n","18","--no-pager","-o","cat",timeout=6).stdout.strip()[-1600:]
 except Exception:return ""
def wait(u,secs=12):
 end=time.monotonic()+secs
 while time.monotonic()<end:
  if active(u):return True
  time.sleep(.4)
 return False
def start(u,retries=5):
 last=""
 for _ in range(retries):
  run("systemctl","reset-failed",u,timeout=5);p=run("systemctl","start",u,timeout=20)
  if p.returncode==0 and wait(u):return True,""
  last=(p.stderr or p.stdout or journal(u) or "serviço não permaneceu ativo").strip();time.sleep(1)
 return False,last or journal(u)
def mqtt_ready(ensure=False):
 try:
  args=["/usr/local/sbin/2pny-mqtt-preflight","--quiet"]
  if ensure:args+=["--ensure-service","--attempts","12","--delay","0.4","--service-wait","10"]
  p=run(*args,timeout=28 if ensure else 6)
  return p.returncode==0,(p.stderr or p.stdout or "").strip()
 except Exception as exc:return False,str(exc)
def udp_listener(port):
 try:
  out=run("ss","-H","-lun",timeout=4).stdout
  for raw in out.splitlines():
   cols=raw.split()
   if len(cols)>=5 and (cols[3].endswith(":"+str(port)) or cols[3].endswith("]:"+str(port))):return True
 except Exception:pass
 return False
def wait_bridge(proto,secs=15):
 port=20010 if proto=="DSTAR" else 4200 if proto=="YSF" else 0
 if not port:return True,""
 end=time.monotonic()+secs
 while time.monotonic()<end:
  if udp_listener(port):return True,""
  time.sleep(.5)
 return False,f"bridge UDP local {port} não ficou pronta"
def wait_prereqs(secs=55):
 end=time.monotonic()+secs;last=""
 while time.monotonic()<end:
  serial_ok=True;port=""
  try:
   cp=configparser.ConfigParser(interpolation=None,strict=False);cp.optionxform=str;cp.read(HOSTCFG)
   port=cp.get("Modem","UARTPort",fallback=cp.get("Modem","Port",fallback="")).strip()
   if port.startswith("/dev/"):serial_ok=Path(port).exists()
  except Exception:serial_ok=False
  mqtt_ok,mqtt_detail=mqtt_ready()
  if serial_ok and mqtt_ok:return True,""
  last=("serial "+(port or "—")+" indisponível" if not serial_ok else (mqtt_detail or "MQTT não respondeu no endpoint configurado"))
  time.sleep(1)
 return False,last

def action():
 if len(sys.argv)>1:return sys.argv[1].strip().lower()
 try:return str(json.loads(REQ.read_text()).get("action") or "").strip().lower()
 except Exception:return ""
def config():
 try:return json.loads(CFG.read_text())
 except Exception:return {}
def display(st,msg):
 try:subprocess.run(["/usr/local/sbin/2pny-display-status",st,msg],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=4)
 except Exception:pass
RUN.mkdir(parents=True,exist_ok=True)
with open(LOCK,"w") as lk:
 fcntl.flock(lk,fcntl.LOCK_EX);a=action();RESTORING=(a=="restore")
 if a=="off":
  DISABLED.write_text("1\n");os.chmod(DISABLED,0o600)
  for u in GATEWAYS.values():run("systemctl","stop",u,timeout=10)
  run("systemctl","stop",HOST,timeout=10);ok=not active(HOST);display("warning","Operacional desligado")
  result({"ok":ok,"operational":False,"state":"disabled","message":"Operacional desligado." if ok else "MMDVMHost não confirmou parada."});raise SystemExit(0 if ok else 1)
 if a not in ("on","restore"):
  result({"ok":False,"error":"ação operacional inválida"});raise SystemExit(2)
 if a=="restore" and DISABLED.exists():
  result({"ok":True,"operational":False,"state":"disabled","reason":"user_disabled","message":"Operacional permaneceu desligado por escolha do usuário."});raise SystemExit(0)
 if not RF.exists():
  result({"ok":False,"operational":False,"error":"RF ainda não foi configurada."});raise SystemExit(3)
 c=config();proto=str(c.get("protocol") or "").upper();unit=GATEWAYS.get(proto)
 if not unit:
  result({"ok":False,"operational":False,"error":"Perfil/protocolo salvo inválido."});raise SystemExit(4)
 if a=="on":
  try:DISABLED.unlink()
  except FileNotFoundError:pass
 ready,detail=mqtt_ready(True)
 if not ready:
  result({"ok":False,"operational":False,"protocol":proto,"error":"MQTT local não ficou pronto.","detail":detail});raise SystemExit(5)
 ready,detail=wait_prereqs()
 if not ready:
  result({"ok":False,"operational":False,"protocol":proto,"error":"Dependências do rádio não ficaram prontas.","detail":detail});raise SystemExit(5)
 run("systemctl","enable",HOST,timeout=8)
 ok,detail=start(HOST)
 if not ok:
  result({"ok":False,"operational":False,"protocol":proto,"service":HOST,"error":"MMDVMHost não iniciou.","detail":detail or journal(HOST)});raise SystemExit(6)
 for p,u in GATEWAYS.items():
  if p==proto:run("systemctl","enable",u,timeout=8)
  else:run("systemctl","disable",u,timeout=8);run("systemctl","stop",u,timeout=8)
 ok,detail=start(unit)
 if not ok:
  result({"ok":False,"operational":False,"protocol":proto,"service":unit,"mmdvmhost_active":active(HOST),"error":"Gateway do perfil salvo não iniciou.","detail":detail or journal(unit)});raise SystemExit(7)
 bridge_ok,bridge_detail=wait_bridge(proto)
 if not bridge_ok:
  run("systemctl","stop",unit,timeout=8)
  result({"ok":False,"operational":False,"protocol":proto,"service":unit,"mmdvmhost_active":active(HOST),"error":"Gateway iniciou, mas a bridge local não ficou pronta.","detail":bridge_detail});raise SystemExit(8)
 display("ready","Operacional restaurado")
 try:subprocess.run(["systemctl","try-restart","2pny-display-core.service"],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=8)
 except Exception:pass
 result({"ok":True,"operational":True,"state":"active","protocol":proto,"gateway":unit,"mmdvmhost_active":True,"gateway_active":True,"message":"Operacional restaurado com o perfil "+proto+"."})
