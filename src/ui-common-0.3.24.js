(function(){
  ...
  function ensureTools(active){
    ...
    getj('/api/language',null,3500).then(function(v){var lang=v&&v.language;if(/^(pt|en|es)$/.test(lang||'')){if(window.PNYSetLanguage)window.PNYSetLanguage(lang,false);else{localStorage.setItem('pu2pny-language',lang);if(q('language'))q('language').value=lang;if(window.translateNavigation)window.translateNavigation()}}}).catch(function(){})
    if(!q('modeToggle')){
  ...
  var connectivityWatchStarted=false,offlineHits=0;
  function startConnectivityWatch(){
    if(connectivityWatchStarted)return;connectivityWatchStarted=true;
    async function check(){
      if(document.hidden)return;
      try{
        var c=await getj('/api/connectivity',null,4000),old=q('pnyInternetAlert');
        if(c.internet){offlineHits=0;if(old)old.remove();return}
        offlineHits++;
        if(offlineHits<2||old||q('pnyOperationOverlay'))return;
        var n=document.createElement('div');n.id='pnyInternetAlert';n.setAttribute('role','alert');
        n.style.cssText='position:fixed;left:50%;top:78px;transform:translateX(-50%);z-index:4100;width:min(620px,calc(100vw - 28px));padding:13px 15px;border:1px solid var(--red);border-radius:14px;background:var(--card);box-shadow:var(--shadow)';
        n.innerHTML='<b>Internet desconectada</b><div class="muted" style="margin-top:4px">Verifique o cabo ou as redes Wi‑Fi salvas. O painel local e a configuração continuam disponíveis.</div>';
        document.body.appendChild(n)
      }catch(_){}
    }
    setTimeout(check,4000);setInterval(check,10000);document.addEventListener('visibilitychange',function(){if(!document.hidden)check()})
  }
  ...
  function nav(active){var items=[['/dashboard','Ao Vivo','live'],['/internet','Internet','internet'],['/hotspot','Protocolos','hotspot'],['/direct','Direct','direct'],['/aprs','APRS','aprs'],['/history','Histórico','history'],['/display','Display','display'],['/system','Sistema','system']];var n=q('mainNav');if(n)n.innerHTML=items.map(function(x){return '<a '+(x[2]===active?'class="active" ':'')+'href="'+x[0]+'">'+x[1]+'</a>'}).join('');ensureTools(active);startClock();startAPRSWatch();startConnectivityWatch();setTimeout(enhanceCurrentPage,0)}
  var clockStarted=false,clockServerUTC=0,clockSyncAt=0,clockTZ='',clockDSTManual=false,clockAutoTZTried=false;
  function renderClock(){
    var l=q('clockLocal'),u=q('clockUTC');if(!l&&!u)return;
    var ms=clockServerUTC?clockServerUTC+(Date.now()-clockSyncAt):Date.now();if(clockDSTManual)ms+=3600000;var d=new Date(ms);
    if(l){try{l.textContent=new Intl.DateTimeFormat(pnyLocale(),{timeZone:clockTZ||undefined,hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).format(d)}catch(e){l.textContent=d.toLocaleTimeString()}}
    if(u)u.textContent=d.toISOString().slice(11,19)+' UTC'
  }
  async function syncClock(){
    try{var s=await getj('/api/system',null,4500),utc=Date.parse(s.utc||'');if(Number.isFinite(utc)){clockServerUTC=utc;clockSyncAt=Date.now()}clockTZ=s.timezone||clockTZ;clockDSTManual=!!(s.clock_settings&&s.clock_settings.dst_manual);
      if(!clockAutoTZTried){clockAutoTZTried=true;var cs=s.clock_settings||{},zone='';try{zone=Intl.DateTimeFormat().resolvedOptions().timeZone||''}catch(_){}
        if(zone&&cs.timezone_source!=='manual'&&(s.timezone!==zone||cs.timezone_source!=='browser-iana')){
          getj('/api/system',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'auto-time',timezone:zone,dst_manual:!!cs.dst_manual}),pnySilent:true},16000).then(function(){clockTZ=zone;setTimeout(syncClock,300)}).catch(function(){})
        }
      }}catch(e){}
    renderClock()
  }
  ...
  function applyLiveHealth(d){
    ...
    var rfCard=q('pnyRFHealth');if(dir==='RF'&&rv!=null){if(rfCard)rfCard.classList.remove('hidden');q('pnyRFValue').textContent=Math.round(Number(rv))+' dBm';q('pnyRFBar').style.width=rfPct(rv)+'%';q('pnyRFMeta').textContent=(ber!=null?'BER '+Number(ber).toFixed(1)+'% · ':'')+(a.protocol||'RF')}
    else{if(rfCard)rfCard.classList.add('hidden');q('pnyRFValue').textContent='—';q('pnyRFBar').style.width='0%';q('pnyRFMeta').textContent='Aguardando RX do rádio'}
  }
  var pnyLiveES=null;
  ...
  function realValue(v){v=String(v==null?'':v).trim().toLowerCase();return !!v&&v!=='—'&&v!=='--'&&v!=='indisponível'&&v!=='não verificado'}
  function realOnly(id,show){var el=typeof id==='string'?q(id):id;if(!el)return;var target=el.closest('[data-real-only],.row,.metric,.fact,.pny-healthitem')||el;var yes=show==null?realValue(el.textContent):!!show;target.classList.toggle('pny-real-hidden',!yes)}
  function sweepRealOnly(root){(root||document).querySelectorAll('[data-real-only],.statusgrid>div,.metrics>.metric,.facts>.fact,.rows>.row').forEach(function(n){if(n.hasAttribute('data-real-keep'))return;var v=n.querySelector('b,code,[data-real-value]')||n;realOnly(v)})}
  var realObserver=null;function observeRealOnly(){sweepRealOnly(document);if(realObserver||!window.MutationObserver)return;var pending=false;realObserver=new MutationObserver(function(){if(pending)return;pending=true;requestAnimationFrame(function(){pending=false;sweepRealOnly(document)})});realObserver.observe(document.body,{subtree:true,childList:true,characterData:true})}
  function enhanceCurrentPage(){ensureGlobalCSS();observeRealOnly();enhanceEmbedded();ensureHealth();enhanceHotspot();if(location.pathname==='/dashboard'){loadConnectivity();startHealthStream();setInterval(function(){if(!document.hidden)loadConnectivity()},15000)}}

  window.PNY={q:q,esc:esc,getj:getj,setTheme:setTheme,initTheme:initTheme,flagUrl:flagUrl,fmtTime:fmtTime,locale:pnyLocale,formatNumber:function(v,o){try{return new Intl.NumberFormat(pnyLocale(),o||{}).format(v)}catch(e){return String(v)}},nav:nav,startClock:startClock,operation:operation,realOnly:realOnly,sweepRealOnly:sweepRealOnly};
