#!/usr/bin/env python3
"""PU2PNY-OS 0.3.17 privileged timezone helper.

Consumes one uniquely named request from /run/2pny. No shell, no arbitrary
...
def main():
    ...
    try:
        ...
        if str(req.get("request_id") or "")!=request_id:
            raise ValueError("request-id de fuso não confere")
        tz=str(req.get("timezone") or "").strip()
        if not tz or ".." in tz or tz.startswith("/") or not re.fullmatch(r"[A-Za-z0-9_+.-]+(?:/[A-Za-z0-9_+.-]+)+",tz):
        ...
        if p.returncode:
            detail=(p.stderr or p.stdout or "timedatectl falhou").strip()[-800:]
            raise RuntimeError("timedatectl: "+detail)

        effective=effective_timezone()
        if effective!=tz:
            raise RuntimeError(f"fuso não confirmado: solicitado={tz}, efetivo={effective or '—'}")

        # /etc/timezone is informational on Debian; keep it coherent only
        # after timedated confirmed the effective zone.
