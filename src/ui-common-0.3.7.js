(function(){
  ...
  function nav(active){var items=[['/dashboard','Ao Vivo','live'],['/internet','Internet','internet'],['/hotspot','Hotspot','hotspot'],['/protocols','Protocolos','protocols'],['/direct','Direct','direct'],['/aprs','APRS / D-PRS','aprs'],['/history','Histórico','history'],['/display','Display','display'],['/system','Sistema','system']];var n=q('mainNav');if(!n)return;n.innerHTML=items.map(function(x){return '<a '+(x[2]===active?'class="active" ':'')+'href="'+x[0]+'">'+x[1]+'</a>'}).join('');ensureTools(active);startAPRSWatch()}
