...
END_RE = re.compile(
    ...
    r"(?:end of voice transmission|voice transmission lost|watchdog has expired).*?"
    r"([0-9.]+) seconds(?:, ([0-9.]+)% packet loss)?"
    r"(?:, BER: ([0-9.]+)%)?(?:, RSSI: ([^,]+? dBm))?$", re.I
)
    ...
    def ingest(self, message: str, ts: float | None = None, gateway=False):
        ...
        if end:
            ...
            result = dict(current or {
                ...
                "source": "", "target": "", "started_at": iso(ts - float(end.group(3))),
                "started_unix_ms": round((ts - float(end.group(3))) * 1000),
            })
            result.update({"event": "end", "ended_at": iso(ts), "duration": float(end.group(3))})
            if end.group(4) is not None:
                result["loss"] = float(end.group(4))
            if end.group(5) is not None:
                result["ber"] = float(end.group(5))
            if end.group(6) is not None:
                values = [float(x) for x in re.findall(r"-?\d+(?:\.\d+)?", end.group(6))]
                if values:
