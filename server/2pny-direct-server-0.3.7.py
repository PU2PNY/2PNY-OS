#!/usr/bin/env python3
"""PU2PNY Direct rendezvous/relay server 0.3.7.
...
class DirectServer:
    ...
    def handle(self,raw,addr):
        ...
        if t=="relay":
            if not CALL_RE.fullmatch(to):return
            dest=self.nodes.get(to);pkt=str(m.get("packet") or "")
            if not dest or not pkt or len(pkt)>90000:return
            self.send(dest["addr"],{"t":"relay-data","from":fr,"to":to,"packet":pkt,"ts":int(time.time())});self.relayed+=1
            return

    def serve(self):
        logging.info("PU2PNY Direct rendezvous/relay UDP %s",self.sock.getsockname())
        last=time.monotonic()
        while True:
